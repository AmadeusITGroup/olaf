---
name: orchestrator-m-medium-workflow
description: Master/orchestrator workflow that coordinates the Medium (M) change sub-workflows 2 → 3 → 4
tags: [workflow, orchestrator, medium, governance, coordination]
---
...
## orchestrator-m-medium-workflow: Medium Changes Orchestrator Workflow
...
## Template Variables
- `[WORKFLOW_NAME]`: orchestrator-m-medium-workflow
- `[WORKFLOW_DESCRIPTION]`: Medium (M) changes with standard governance
- `[SUB_WORKFLOW_NAME]`: Design | Planning | Implementation
- `[sub-workflow-file]`: treat-change-request/workflows/M/workflow-2-design | treat-change-request/workflows/M/workflow-3-planning | treat-change-request/workflows/M/workflow-4-implementation
- `[descriptive-orchestrator-log-name]`: m-orchestrator-execution-log
- `[descriptive-final-output-name]`: m-final-output
- `[sub-workflow-output-file]`: design artifacts | planning artifacts | implementation artifacts
- `[orchestrator-state-name]`: orchestrator-m-state
- `[unique-execution-id]`: m-[timestamp]
...
## Workflow Overview
Handles Medium (M) changes involving 2-3 services/modules and 5-10 days effort. Executes three sub-workflows (Design → Planning → Implementation) with required validations and approvals at each stage.

## Prerequisites
- Receives context package from Orchestrator-0-Router containing issue IDs, size classification (M), rationale, technical scope, and router artifacts
- Reviewers identified for design review and plan baseline
...
- Configuration Input: Review gates and checklists for Design, Planning, Implementation
- Input Format: Markdown/YAML for artifacts, JSON for orchestrator state/log

## Output Specifications
- Orchestrator Log: `m-orchestrator-execution-log.json`
- Final Consolidated Output: `m-final-output.json`
...
- Tracking File: `m-progress-tracking.md`
...
- Type: Sequential
- Prompt/Workflow: `[id:prompts_dir]treat-change-request/workflows/M/workflow-2-design.md`
- Input Requirements:
  - Primary: Context package from router and router artifacts
  - From Previous: N/A (first step)
- Output Produced: Design artifacts (technical design and review documentation)
- Description: Produces detailed technical design
...
- Type: Sequential
- Prompt/Workflow: `[id:prompts_dir]treat-change-request/workflows/M/workflow-3-planning.md`
- Input Requirements:
...
- Type: Sequential
- Prompt/Workflow: `[id:prompts_dir]treat-change-request/workflows/M/workflow-4-implementation.md`
- Input Requirements:
  - Primary: Artifacts from Planning
  - Additional: Test plan and validation checklist
...
- All sub-workflow files exist in `[id:prompts_dir]treat-change-request/workflows/M/`
...
### Orchestrator State File: `orchestrator-m-state.json`
```json
{
  "orchestrator_name": "orchestrator-m-medium-workflow",
  "execution_id": "m-[timestamp]",
  ...
  "consolidation_rules": {
    "final_output_structure": {
      "orchestrator_metadata": {
        "execution_summary": "from orchestrator-m-state.json",
